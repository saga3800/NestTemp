...
export default class GeneralUtil {
  ...
  public static async convertXmlToJson(xml: any): Promise<any> {
    if (GeneralUtil.validateValueRequired(xml)) {
      ...
      return parser
        ...
        .then((result) => {
          console.log(
            'Result JSON transform from XML => \n',
  ...
  public static validateChannel(channel: string): boolean {
    if (Echannel[channel]) return true;
    else
      throw new BusinessException(
        HttpStatus.BAD_REQUEST,
        channel == undefined
          ? 'Debe indicar un canal válido.'
          : `${channel} no es un canal válido.`,
        false,
  ...
  public static serializeQs = (parameters) => {
    return Object.keys(parameters)
      .map((p, i) => (i == 0 ? '?' : '&') + p + '=' + parameters[p])
      .reduce((a, b) => a + b, '');
  };
}
